    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="form-group" >
                    {{ Form::label('State : ') }}
                    {{Form::select('state', array('Select State')+$states,0,array('id'=>'state_drop','class'=>'form-control'))}}
                    {{ $errors->first('state','<p class="text-red">:message</p>'); }}
                </div>
                <div class="form-group city hidden" >
                    {{ Form::label('City : ') }}
                    @foreach($cities as $key=>$city_list)
                    {{Form::select('city', array('Select City')+$city_list,0,array('id'=>str_replace(' ','_',$key),'class'=>'form-control city_drop hidden','disabled'=>'disabled'))}}
                    @endforeach
                    {{ $errors->first('city','<p class="text-red">:message</p>'); }}
                </div>
                <div class="form-group" >
                    {{ Form::label('Link : ') }}
                    {{ Form::text('link','http://',array('class'=>'form-control','id'=>'link')) }}
